...
export function GrantsPage() {
  ...
  return (
    <Box>
      ...
      <Paper component="form" variant="outlined" sx={{ p: 2.5, mb: 3 }} onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters({ ...draft }); }}>
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid size={{ xs: 6, md: 2 }}><TextField select size="small" fullWidth label="Статус" value={draft.status} onChange={(e) => setDraft({ ...draft, status: e.target.value })}><MenuItem value="">Все</MenuItem><MenuItem value="OPEN">Приём заявок</MenuItem><MenuItem value="PUBLISHED">Опубликован</MenuItem></TextField></Grid>
          <Grid size={{ xs: 6, md: 2 }}><TextField size="small" fullWidth label="Категория" value={draft.category} onChange={(e) => setDraft({ ...draft, category: e.target.value })} /></Grid>
          <Grid size={{ xs: 6, md: 2 }}><TextField size="small" fullWidth label="Дедлайн до" type="date" value={draft.deadline_to} onChange={(e) => setDraft({ ...draft, deadline_to: e.target.value })} slotProps={{ inputLabel: { shrink: true } }} /></Grid>
          <Grid size={{ xs: 6, md: 2 }}><TextField select size="small" fullWidth label="Сортировка" value={draft.ordering} onChange={(e) => setDraft({ ...draft, ordering: e.target.value })}><MenuItem value="-created_at">Сначала новые</MenuItem><MenuItem value="end_date">По сроку</MenuItem><MenuItem value="-max_amount">По сумме</MenuItem><MenuItem value="title">По названию</MenuItem></TextField></Grid>
          <Grid size={{ xs: 6, md: 2 }}><Button fullWidth type="submit" variant="contained">Применить</Button></Grid>
        </Grid>
      </Paper>
              ...
              <Grid key={grant.id} size={{ xs: 12, md: 6 }}>
                <Card variant="outlined" sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
                  <CardContent sx={{ flex: 1 }}>
                    ...
                    <Stack direction={{ xs: "column", sm: "row" }} spacing={{ xs: 0.5, sm: 3 }} sx={{ mt: 2 }}><Typography variant="body2"><b>До:</b> {dateFormat.format(new Date(grant.end_date))}</Typography><Typography variant="body2"><b>Сумма:</b> {currency.format(Number(grant.max_amount))}</Typography></Stack>
                  </CardContent>
                  <CardActions><Button component={RouterLink} to={`/grants/${grant.id}`}>Подробнее</Button></CardActions>
                </Card>
              </Grid>
            ))}
          </Grid>
