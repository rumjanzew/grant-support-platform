...
export function AdminGrantsPage() {
  ...
  return <>
    ...
    <Paper component="form" variant="outlined" sx={{ p: 2, mb: 3 }} onSubmit={(event) => { event.preventDefault(); setPage(1); setQuery({ search, status: statusValue }); }}><Grid container spacing={2}><Grid size={{ xs: 12, sm: 6 }}><TextField fullWidth size="small" label="Поиск" value={search} onChange={(event) => setSearch(event.target.value)} /></Grid><Grid size={{ xs: 7, sm: 3 }}><TextField select fullWidth size="small" label="Статус" value={statusValue} onChange={(event) => setStatusValue(event.target.value)}><MenuItem value="">Все</MenuItem>{["DRAFT", "PUBLISHED", "OPEN", "CLOSED", "ARCHIVED"].map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 5, sm: 3 }}><Button fullWidth type="submit" variant="outlined">Найти</Button></Grid></Grid></Paper>
