...
export function AdminGrantsPage() {
  ...
  return <>
    ...
    {loading ? <LoadingState /> : !data?.results.length ? <EmptyState title="Грантов нет" description="Создайте первый грант или измените параметры поиска." /> : <><Grid container spacing={2}>{data.results.map((grant) => <Grid key={grant.id} size={{ xs: 12, md: 6 }} sx={{ display: "flex" }}><Card variant="outlined" sx={{ width: "100%", height: "100%", display: "flex", flexDirection: "column" }}><CardContent sx={{ flex: 1 }}><Stack direction="row" justifyContent="space-between" gap={2}><Typography variant="overline">{grant.code}</Typography><StatusChip status={grant.status} /></Stack><Typography variant="h6" sx={{ mt: 1 }}>{grant.title}</Typography><Typography color="text.secondary">{grant.category}</Typography></CardContent><CardActions sx={{ mt: "auto" }}><Button component={RouterLink} to={`/admin/grants/${grant.id}/edit`} startIcon={<EditIcon />}>Редактировать</Button><Button component={RouterLink} to={`/grants/${grant.id}`}>Открыть</Button></CardActions></Card></Grid>)}</Grid>{data.count > 10 && <Stack alignItems="center" sx={{ mt: 3 }}><Pagination page={page} count={Math.ceil(data.count / 10)} onChange={(_, value) => setPage(value)} /></Stack>}</>}
  </>;
}
