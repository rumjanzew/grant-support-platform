...
import type { AdministratorApplication, PaginatedResponse, UserSummary } from "../../types";
...
export function AdminApplicationsPage() {
  ...
  return <>
    ...
    <Paper component="form" variant="outlined" sx={{ p: 2, mb: 3 }} onSubmit={(event) => { event.preventDefault(); setPage(1); setFilters({ search, status: statusValue }); }}><Grid container spacing={2}><Grid size={{ xs: 12, sm: 6 }}><TextField fullWidth size="small" label="Номер, проект или организация" value={search} onChange={(event) => setSearch(event.target.value)} /></Grid><Grid size={{ xs: 7, sm: 3 }}><TextField select fullWidth size="small" label="Статус" value={statusValue} onChange={(event) => setStatusValue(event.target.value)}><MenuItem value="">Все</MenuItem>{["SUBMITTED", "REVISION_SUBMITTED", "UNDER_REVIEW", "APPROVED", "REJECTED", "REVISION_REQUIRED"].map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 5, sm: 3 }}><Button fullWidth type="submit" variant="outlined">Применить</Button></Grid></Grid></Paper>
