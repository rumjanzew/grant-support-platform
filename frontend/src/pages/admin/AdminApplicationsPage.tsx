...
export function AdminApplicationsPage() {
  ...
  return <>
    ...
    {loading ? <LoadingState /> : !data?.results.length ? <EmptyState title="Заявок нет" description="По выбранным условиям заявки не найдены." /> : <><Grid container spacing={2}>{data.results.map((application) => <Grid key={application.id} size={{ xs: 12, md: 6 }} sx={{ display: "flex" }}><Card variant="outlined" sx={{ width: "100%", height: "100%", display: "flex", flexDirection: "column" }}><CardContent sx={{ flex: 1 }}><Stack direction="row" justifyContent="space-between" gap={2}><Typography variant="overline">{application.application_number ?? "Без номера"}</Typography><StatusChip status={application.status} /></Stack><Typography variant="h6" sx={{ mt: 1 }}>{application.project_name}</Typography><Typography color="text.secondary">{application.organization_name} · {application.grant_title}</Typography><Typography sx={{ mt: 1 }}>{currency.format(Number(application.requested_amount))}</Typography>{application.assignment && <Alert severity={application.assignment.status === "ACTIVE" ? "info" : "success"} sx={{ mt: 2 }}>Эксперт: {application.assignment.expert_name}</Alert>}{application.attachments.length ? <Stack spacing={1} sx={{ mt: 2 }}><Typography variant="subtitle2">Документы ({application.attachments.length})</Typography><AttachmentList applicationId={application.id} attachments={application.attachments} /></Stack> : <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>Документы не приложены.</Typography>}</CardContent>{assignable.has(application.status) && <CardActions sx={{ mt: "auto" }}><Button startIcon={<PersonAddIcon />} onClick={() => void openAssignment(application)}>Назначить эксперта</Button></CardActions>}</Card></Grid>)}</Grid>{data.count > 10 && <Stack alignItems="center" sx={{ mt: 3 }}><Pagination page={page} count={Math.ceil(data.count / 10)} onChange={(_, value) => setPage(value)} /></Stack>}</>}
