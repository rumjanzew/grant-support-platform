...
export function ResetPasswordPage() {
  ...
  return <Paper variant="outlined" sx={{ maxWidth: 520, mx: "auto", p: { xs: 3, sm: 4 } }}><Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}><BrandLogo to="/" /></Box><Typography variant="h4" component="h1">Новый пароль</Typography><Typography color="text.secondary" sx={{ mt: 1, mb: 3 }}>Придумайте новый безопасный пароль для аккаунта.</Typography><Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate><Stack spacing={2.5}>{error && <Alert severity="error">{error}</Alert>}<PasswordField label="Новый пароль" autoComplete="new-password" fullWidth error={Boolean(errors.password)} helperText={errors.password?.message ?? "Не менее 8 символов"} {...register("password", { required: "Введите пароль", minLength: { value: 8, message: "Минимум 8 символов" } })} /><PasswordField label="Повторите пароль" autoComplete="new-password" fullWidth error={Boolean(errors.password_confirm)} helperText={errors.password_confirm?.message} {...register("password_confirm", { required: "Повторите пароль", validate: (value) => value === password || "Пароли не совпадают" })} /><Button type="submit" variant="contained" size="large" disabled={isSubmitting}>{isSubmitting ? "Сохраняем…" : "Установить новый пароль"}</Button></Stack></Box></Paper>;
}
